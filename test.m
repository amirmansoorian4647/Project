...
sources_num = 2;
dim = 2;
r0 = [27;11.137;0];  % initial loc of array [m] (location that for first time receive a signal)
r_prim1 = [1;0.56;1]; % source 1 loc [m]
r_prim2 = [-5.067;-17.23;2]; % source 2 loc [m]
r_prim = [r_prim1,r_prim2];
v = [0.7;1.5;1];     % array velocity [m/sec]

d = [0;0.12;0.25;0.37;0.45;0.67];      % array dist elements [m] (array is linier)
fc = 5e6;          % carrier freq [Hz]
Diversity = @(theta_s, phi_s) 1;% source diversity
...
S2 = sin(2*pi*14*time);% source 2 signal
S = [S1;S2];
%% in t = 0.2:3:15 [sec]
...
for t = receiving_times
   ...
   %-------------------------------------------------------------Estimation
   phi_theta = MUSIC(received_signal,d,fc);
   
   time_txt = num2str(t);
   disp("-------------------------At t = "+time_txt+" [sec]-------------------------")
   disp('Relative Angles [deg] = ')
   disp(phi_theta*180/pi)
   L = length(phi_theta);
   INDEX = [];
   for i = 1:L
       u = [cos(phi_theta(i));sin(phi_theta(i))];
       if t==receiving_times(1)
           A(:,:,i) = eye(dim)-u*u';
           B(:,i) = (eye(dim)-u*u')*r;
           INDEX = [INDEX,i];
       else
           [~,idx] = min(abs(theta_old-phi_theta(i)));
